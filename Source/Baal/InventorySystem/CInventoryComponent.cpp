#include "InventorySystem/CInventoryComponent.h"
...
void UCInventoryComponent::AddItem(FName Id, int Amount)
{
	...
	if (Items.Contains(Id))
	{
		...
		RequiredSlotCount = TotalSlotCount - CurrentSlotCount;
	}
	else
	{
		RequiredSlotCount = FMath::CeilToInt((float)Amount /MaxStackAmount);
	}
	
	// Check if there is enough slots in the inventory
	if (MaxSlotCount < FilledSlotCount + RequiredSlotCount)
	{
